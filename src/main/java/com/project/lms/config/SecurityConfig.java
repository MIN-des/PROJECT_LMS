...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests() // 모든 사람이 볼 수 있음, 미리보기 추가함 url 설정도 추가함
                .antMatchers("/", "/login", "/meals/**").permitAll()
                .antMatchers("/board", "/board/**").permitAll()
                .antMatchers("schedule/**", "/uploaded-images/**").permitAll()
                .antMatchers("/student/invoices/preview/**", "/invoices/download/**").permitAll()
